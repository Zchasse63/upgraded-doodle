...
// Guarantee: alertOps NEVER throws, regardless of what fails internally.
// Callers should still `void alertOps(...)` (fire-and-forget) to avoid
// adding latency from the throttle DB query, but a defensive outer try/catch
// here means a bad invocation can never break a handler.
export async function alertOps(
  ...
  try {
    ...
    // Throttle by counting recent failed rows for the same event. Failures
    // only — we don't want successful events to drown out real alerts.
    ...
    console.error(JSON.stringify({
      level: "warn",
      msg: "alertOps_swallowed_exception",
